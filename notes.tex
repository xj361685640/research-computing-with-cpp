...
\subsubsection{Build Your Own}\label{build-your-own}
...
\subsubsection{32 or 64 bit}\label{or-64-bit}
...
\subsubsection{Some Better Names}\label{some-better-names}
...
\item
  ...
  point to header file locations and link to the right libraries. An MPI
  program can be (easily) compiled by substituting
...
\begin{Shaded}
\begin{Highlighting}[]
  \KeywordTok{module} \NormalTok{swap compilers/gnu}
  \KeywordTok{module} \NormalTok{unload mpi/intel/2015/update3/intel}
  \KeywordTok{module} \NormalTok{load mpi/openmpi/3.0.0/gnu-4.9.2}
  \KeywordTok{module} \NormalTok{load cmake}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DataTypeTok{int} \NormalTok{MPI_Recv(}\DataTypeTok{void} \NormalTok{*buf, }\DataTypeTok{int} \NormalTok{bufsize, MPI_Datatype datatype, }\DataTypeTok{int} \NormalTok{source, }\DataTypeTok{int} \NormalTok{tag,}
...
\subsubsection{Function signatures}\label{function-signatures}
...
\DataTypeTok{int} \NormalTok{MPI_Irecv(}\DataTypeTok{void} \NormalTok{*buf, }\DataTypeTok{int} \NormalTok{bufsize, MPI_Datatype datatype, }\DataTypeTok{int} \NormalTok{source, }\DataTypeTok{int} \NormalTok{tag,}
              \NormalTok{MPI_Comm comm, MPI_Request *request)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
      ...
      \NormalTok{MPI_Request send_req;}
      \CommentTok{// Everyone sets up their messages to send}
      \DataTypeTok{int} \NormalTok{error = MPI_Isend(}
        \NormalTok{&message, }\DecValTok{1}\NormalTok{, MPI_INT, left, rank, MPI_COMM_WORLD, &send_req);}
      \NormalTok{REQUIRE(error == MPI_SUCCESS);}
      ...
      \DataTypeTok{int} \NormalTok{completed;}
      \NormalTok{error = MPI_Test(&send_req, &completed, MPI_STATUS_IGNORE);}
      \NormalTok{REQUIRE(error ==  MPI_SUCCESS);}
      \NormalTok{REQUIRE(completed == }\KeywordTok{true}\NormalTok{);}
...
\KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{i(}\DecValTok{0}\NormalTok{); i < size; ++i) \{}
    \KeywordTok{if} \NormalTok{(i == }\DecValTok{0} \NormalTok{&& rank == }\DecValTok{0}\NormalTok{) \{ }\CommentTok{/* broadcast */} \NormalTok{\}}
...
\begin{Highlighting}[]
    ...
    \NormalTok{SECTION(}\StringTok{"split 1:2 and keep same process order"}\NormalTok{) \{}
...
\subsubsection{Splitting communicators:
Exercises}\label{splitting-communicators-exercises}
...
\item
  Use one of the collective operations on a single group
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\DataTypeTok{int} \NormalTok{MPI_Reduce(}\DataTypeTok{const} \DataTypeTok{void} \NormalTok{*sendbuf, }\DataTypeTok{void} \NormalTok{*recvbuf, }\DataTypeTok{int} \NormalTok{count,}
               \NormalTok{MPI_Datatype datatype, MPI_Op op, }\DataTypeTok{int} \NormalTok{root,}
               \NormalTok{MPI_Comm comm)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
      ...
      \DataTypeTok{const} \DataTypeTok{int} \NormalTok{TERMS_PER_PROCESS = }\DecValTok{1000}\NormalTok{;}
      \DataTypeTok{double} \NormalTok{my_denominator = rank * TERMS_PER_PROCESS * }\DecValTok{2} \NormalTok{+ }\DecValTok{1}\NormalTok{;}
      \DataTypeTok{double} \NormalTok{sign = }\DecValTok{1}\NormalTok{;}
      \DataTypeTok{double} \NormalTok{my_result = }\FloatTok{0.0}\NormalTok{;}
      \KeywordTok{for} \NormalTok{(}\DataTypeTok{int} \NormalTok{i(}\DecValTok{0}\NormalTok{); i<TERMS_PER_PROCESS; ++i)}
      \NormalTok{\{}
        \NormalTok{my_result += sign / my_denominator;}
        \NormalTok{my_denominator += }\DecValTok{2}\NormalTok{;}
        \NormalTok{sign = -sign;}
      \NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
      ...
      \KeywordTok{if} \NormalTok{(message.size() < (}\DataTypeTok{unsigned}\NormalTok{)size) }\KeywordTok{return}\NormalTok{;}
...
\DataTypeTok{int} \NormalTok{main()}
\NormalTok{\{}
  \NormalTok{FILE* if1 = fopen(}\StringTok{"90stl/cpp/randomNumbers1.txt"}\NormalTok{,}\StringTok{"r"}\NormalTok{);}
  \NormalTok{FILE* if2 = fopen(}\StringTok{"90stl/cpp/randomNumbers2.txt"}\NormalTok{,}\StringTok{"r"}\NormalTok{);}
...
\DataTypeTok{int} \NormalTok{main()}
\NormalTok{\{}
  \NormalTok{std::ifstream if1(}\StringTok{"90stl/cpp/randomNumbers1.txt"}\NormalTok{,std::ifstream::in);}
  \NormalTok{std::ifstream if2(}\StringTok{"90stl/cpp/randomNumbers2.txt"}\NormalTok{,std::ifstream::in);}
...
\DataTypeTok{int} \NormalTok{main()}
\NormalTok{\{}
  \NormalTok{std::ifstream ifs(}\StringTok{"90stl/cpp/particleList.txt"}\NormalTok{,std::ifstream::in);}
  \CommentTok{// Read in the data}
...
\section{Appendix - TMP}\label{appendix---tmp}
...
\KeywordTok{public}\NormalTok{:}
  ...
  \CommentTok{//! Find the distance between 2 indices on a 1d torus}
  ...
  \CommentTok{//! Find the Euclidean distance between points on a torus}
...
\DataTypeTok{void} \NormalTok{Smooth::LayeredUpdate() \{}
  ...
  \CommentTok{// Start synchronising the field}
  \DataTypeTok{auto} \NormalTok{request = WholeFieldNonBlockingSync(field, communicator);}
  \DataTypeTok{auto} \DataTypeTok{const} \NormalTok{start = OwnedStart(Size(), ncomms, rank);}
  \DataTypeTok{auto} \DataTypeTok{const} \NormalTok{end = OwnedStart(Size(), ncomms, rank + }\DecValTok{1}\NormalTok{);}
  \CommentTok{// Determine which locations could be affected by other processes}
  ...
  \CommentTok{// Define a lambda function to update part of our field}
  ...
  \CommentTok{// Update those locations that cannot be affected by other processes}
  ...
  \CommentTok{// Wait for field sync to finish}
  \NormalTok{MPI_Wait(&request, MPI_STATUS_IGNORE);}

  \CommentTok{// Update the remaining locations}
...
\section{Appendix - MPI-I/O}\label{appendix---mpi-io}

\subsection{Appendix - MPI-I/O}\label{appendix---mpi-io-1}
...
\OtherTok{#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"}
...
\OtherTok{#pragma GCC diagnostic pop}
...
\subsubsection{No Longer In Course}\label{no-longer-in-course-2}
